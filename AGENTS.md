...
무엇을 어디에 쓸지 고를 때 보는 표다. 각 문서의 역할은 그 문서 첫 줄에도 적혀 있다.
...
- 문서 첫 줄에 그 문서가 담는 것을 한 줄로 쓴다. 다른 문서로 보내는 안내는 쓰지 않는다. 어디에 무엇이 있는지는 이 표가 답한다.
...
- 어디에 쓸지 애매하면 `docs/PLAN.md` 에 쓴다. 여러 세션이 반복해서 필요로 한다고 드러날 때 위 문서로 올린다.
