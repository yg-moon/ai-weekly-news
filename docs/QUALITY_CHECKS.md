...
무엇이 아닌지로 설명하는 꼴도 함께 본다. 대조 자체가 요점인 문장("개별 인물의 문제가 아니라 절차가 쟁점")과 한계를 밝히는 문장은 그대로 둔다. 설명을 대조로 시작한 것만 고친다.

```bash
grep -n -o '.\{0,30\}\(이지 [^ ]* 아니\|가 아니라\|이 아니라\|하지 않는다\. [^ ]*기 때문\).\{0,30\}' "$W" || echo "(없음)"
```
